...
class Violas extends React.Component {
  ...
  async getSeqNumb(_address) {
    await axios(
      `https://api.violas.io/1.0/violas/seqnum?addr=${_address}`
    ).then((res) => {
      return res.data.data + 1;
    });
  }
  async getViolasCurrencies() {
    await axios("https://api4.violas.io/1.0/violas/currency").then(
      async (res) => {
  ...
  async signTransaction() {
    ...
    this.props.walletConnector
      .signTransaction({
        address: sessionStorage.getItem("violas_address"),
      ...
      <div className="boxs">
        ...
        <div className="tx">
          ...
          <p>
            Select Stable Coin:
            <select
              value={this.state.violas_currency}
              onChange={this.handleChange.bind(this, "violas_currency")}
            >
              {this.state.violas_currencies &&
                this.state.violas_currencies.map((v, i) => {
                  return (
        ...
        <div className="tx">
          ...
          <p>
            ...
            <input
              type="text"
              onChange={this.handleChange.bind(this, "violas_address")}
            />
          </p>
          <p>
            Value:{" "}
            <input
              type="text"
              onChange={this.handleChange.bind(this, "violas_value")}
            />
          </p>
          <select
            value={this.state.violas_currency}
