...
const final_code = async () => {
    let transfer_hex = byte2hex(v2_transfer);
    let transferWithData_hex = byte2hex(v2_transferWithData);
    let publish_hex = byte2hex(v2_publish);
...
const writeToken = async _ => {
    fs.writeFileSync(file1, JSON.stringify(await final_code()));
    fs.writeFileSync(file2, JSON.stringify(await final_code()));
}
